...
def paged_attention(
    ...
    # TODO: Mosaic decode disabled - 1D iota layout inference not yet resolved
    # The issue: Mosaic GPU requires explicit layouts but none work for 1D arrays:
    # - No layout → "Failed to infer" error
    # - WGMMA layout → "Tiling does not apply to (64,1)" error  
    # - WG_SPLAT layout → NotImplementedError in thread_idxs
    # Solution requires either: (1) pre-compute indices outside kernel, or  
    # (2) use full 2D WGMMA masks like JAX attention_mgpu.py
