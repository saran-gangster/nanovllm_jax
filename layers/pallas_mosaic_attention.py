...
def batched_decode_attention_mosaic(
    ...
    def kernel_body(
        ...
        def _compute_wg():
            ...
            # Use broadcasted_iota WITHOUT layout for 1D arrays (let JAX infer)
            row_ids = plgpu.broadcasted_iota(jnp.int32, (block_q,), 0)
            ...
            # Use broadcasted_iota WITHOUT layout for 1D arrays
            row_indices = plgpu.broadcasted_iota(jnp.int32, (block_q,), 0)
            ...
            def kv_loop(kv_step, carry):
                ...
                # Use broadcasted_iota WITHOUT layout for 1D arrays
                col_ids = plgpu.broadcasted_iota(jnp.int32, (block_kv,), 0)
...
def paged_decode_attention_mosaic_v2(
    ...
    def kernel(q_ref, k_cache_ref, v_cache_ref, block_tables_ref,
        ...
        def process_logical_block(logical_block_idx, carry):
            ...
            # Use broadcasted_iota WITHOUT layout for 1D arrays
            seq_indices = plgpu.broadcasted_iota(jnp.int32, (block_q,), 0) + batch_start
            ...
            def process_kv_tile(kv_tile_idx, inner_carry):
                ...
                # Use broadcasted_iota WITHOUT layout for 1D arrays
                kv_positions = plgpu.broadcasted_iota(jnp.int32, (block_kv,), 0) + block_start_pos + kv_pos_in_block
