...
def batched_decode_attention_mosaic(
    ...
    def kernel_body(
        ...
        def _compute_wg():
            ...
            row_ids_2d = plgpu.broadcasted_iota(jnp.int32, (block_q, 1), 0, layout=plgpu.Layout.WGMMA)
            row_ids = row_ids_2d[:, 0]  # Extract 1D from 2D
            ...
            row_indices_2d = plgpu.broadcasted_iota(jnp.int32, (block_q, 1), 0, layout=plgpu.Layout.WGMMA)
            row_indices = row_indices_2d[:, 0]  # Extract 1D from 2D
            ...
            def kv_loop(kv_step, carry):
                ...
                col_ids_2d = plgpu.broadcasted_iota(jnp.int32, (1, block_kv), 1, layout=plgpu.Layout.WGMMA)
                col_ids = col_ids_2d[0, :]  # Extract 1D from 2D
...
def paged_decode_attention_mosaic_v2(
    ...
    def kernel(q_ref, k_cache_ref, v_cache_ref, block_tables_ref,
        ...
        def process_logical_block(logical_block_idx, carry):
            ...
            seq_indices_2d = plgpu.broadcasted_iota(jnp.int32, (block_q, 1), 0, layout=plgpu.Layout.WGMMA)
            seq_indices = seq_indices_2d[:, 0] + batch_start  # Extract 1D from 2D
            ...
            def process_kv_tile(kv_tile_idx, inner_carry):
                ...
                kv_positions_2d = plgpu.broadcasted_iota(jnp.int32, (1, block_kv), 1, layout=plgpu.Layout.WGMMA)
                kv_positions = kv_positions_2d[0, :] + block_start_pos + kv_pos_in_block  # Extract 1D from 2D
