...
class LLMEngine:
    ...
    def __init__(self, model: str, **kwargs):
        ...
        unknown_kwargs = sorted(k for k in kwargs if k not in config_fields)
        if unknown_kwargs:
            unknown = ", ".join(unknown_kwargs)
            raise TypeError(f"Unexpected LLM config arguments: {unknown}")
    ...
    def step(self) -> tuple[list[tuple[int, list[int]]], int]:
        ...
        bt_dirty = self.scheduler._block_tables_dirty
        self.scheduler._block_tables_dirty = False
    ...
    def generate(
        ...
        if not isinstance(sampling_params, list):
            ...
            raise ValueError(
                "sampling_params list length must match the number of prompts"
            )
        
        # Batch-tokenize string prompts to reduce Python/tokenizer overhead.
        prompts_to_add = prompts
        if prompts and isinstance(prompts[0], str):
            ...
            prompts_to_add = tokenized["input_ids"]

        # Add all requests
        for prompt, sp in zip(prompts_to_add, sampling_params, strict=True):
        ...
        # Decode all outputs in one tokenizer call to reduce host-side overhead.
        ...
        outputs = [
            ...
            for text, token_ids in zip(decoded_texts, output_token_ids)
        ]
