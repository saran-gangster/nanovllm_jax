"""Internal tensor-parallel linear layers for JAX with Flax NNX.

This module keeps the tensor-parallel building blocks used by experimental
multi-device work. The supported public runtime uses `tensor_parallel_size=1`,
so these layers are internal infrastructure rather than part of the product
contract.
...
- Uses jax.shard_map for explicit SPMD programming
...
class LinearBase(nnx.Module):
    """Base class for internal tensor-parallel linear layers.
