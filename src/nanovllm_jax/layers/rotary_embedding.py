...
def apply_rotary_emb(
    ...
    """
    # Keep output in the same dtype as `x` (bf16/fp16) for performance.
    # The cached cos/sin tables are float32; cast them to match `x` to avoid
    # upcasting Q/K to float32 (which slows attention and breaks cuDNN FA,
    # since cuDNN does not support fp32 inputs here).
...
def get_rope(
    ...
    if rope_scaling is not None:
        raise ValueError("rope_scaling is not yet supported")
