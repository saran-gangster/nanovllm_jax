...
class Sampler(nnx.Module):
    ...
    def __call__(
        ...
        # Fast path for near-greedy decode: skip RNG and temperature division.
        # This preserves exact argmax behavior and avoids unnecessary work.
        all_greedy = jnp.all(temperatures < 1e-6)
        key = self.rngs()
        ...
        def _sample(_):
            # Gumbel-max trick: argmax(logits/T + gumbel_noise) is equivalent
            # to sampling from softmax(logits/T) but avoids explicit softmax.
            uniform = jax.random.uniform(key, logits.shape, minval=1e-10, maxval=1.0)
            gumbel_noise = -jnp.log(-jnp.log(uniform))
            return jnp.argmax(logits / temperatures[:, None] + gumbel_noise, axis=-1)

        return jax.lax.cond(all_greedy, _greedy, _sample, operand=None)
