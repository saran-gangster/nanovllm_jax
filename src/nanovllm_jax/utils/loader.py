...
def load_model(model: nnx.Module, path: str, *, strict: bool = True):
    ...
    Args:
        ...
        strict: Raise if any checkpoint tensor cannot be matched or loaded.
    """
        ...
        with safe_open(file, framework="np", device="cpu") as f:
            for weight_name in f.keys():
                try:
                    ...
                    failures.append(f"{weight_name}: {e}")
                    skipped_count += 1

    if loaded_count == 0:
        raise ValueError(f"No checkpoint weights were loaded from {path}")

    if strict and (skipped_weights or failures):
        details = []
        if skipped_weights:
            sample = ", ".join(skipped_weights[:5])
            details.append(
                f"{len(skipped_weights)} checkpoint tensors did not match model modules "
                f"(examples: {sample})"
            )
        if failures:
            sample = "; ".join(failures[:3])
            details.append(
                f"{len(failures)} tensors raised load errors (examples: {sample})"
            )
        raise ValueError(
            f"Checkpoint load for {path} was incomplete. " + " ".join(details)
        )

    logger.info(
        "Loaded %s weights from %s (%s skipped)",
        loaded_count,
        path,
        skipped_count,
    )
...
def load_model_sharded(
    ...
    if hasattr(model, 'tp_rank'):
        if model.tp_rank != tp_rank:
            raise ValueError(f"Model tp_rank {model.tp_rank} != requested tp_rank {tp_rank}")
    if hasattr(model, 'tp_size'):
        if model.tp_size != tp_size:
            raise ValueError(f"Model tp_size {model.tp_size} != requested tp_size {tp_size}")
